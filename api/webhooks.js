  ...
  return (
    <div className="app">
      <div className="top">
        ...
        <div className="muted">Public web app (static UI + serverless webhook)</div>
      </div>
      <div className="wrap">
        <div className="nav">
...
function Dashboard({kpis, scorecards}){
  ...
  return (
    <div className="grid grid-3 grid">
      <div className="card"><div className="muted">Calls Reviewed</div><div style="font-size:28px;font-weight:700;">{kpis.reviewed}</div><div className="muted" style="margin-top:6px;">Last 30 days</div></div>
      <div className="card"><div className="muted">Avg Score</div><div style="font-size:28px;font-weight:700;">{kpis.avg}/25</div><div className="muted" style="margin-top:6px;">Across all criteria</div></div>
      <div className="card"><div className="muted">Win-Rate (est.)</div><div style="font-size:28px;font-weight:700;">{kpis.winRate}%</div><div className="muted" style="margin-top:6px;">Based on coaching quality index</div></div>
      <div className="card" style="grid-column:span 2;"><div style="font-weight:600;margin-bottom:8px;">Weekly Trend (demo)</div><div className="muted">Charts omitted in single-file build.</div></div>
      <div className="card"><div style="font-weight:600;margin-bottom:8px;">Recent Scorecards</div>
        <div className="list">
          {latest.length===0 ? <div className="muted">No reviews yet. Add one in Scorecards.</div> :
            latest.map((r,i)=>(
              <div key={i} className="row" style={{justifyContent:"space-between",border:"1px solid var(--border)",padding:8,borderRadius:12}}>
                <div><div style={{fontWeight:600}}>{r.rep||"Rep"} • {r.account||"Account"}</div><div className="muted">{new Date(r.date).toLocaleString()}</div></div>
                <div style={{textAlign:"right"}}><div style={{fontWeight:700}}>{r.total}/25</div><div className="muted">{(r.notes||"").slice(0,28)}</div></div>
              </div>
            ))
          }
        </div>
...
function Drills({drills, onAdd, onEvent}){
  ...
  useEffect(()=>{
    if(!running) return; const d = drills.find(x=>x.id===running); if(!d) return;
    setRemaining(d.durationSec); onEvent?.("drill.started",{drillId:d.id,title:d.title,ts:new Date().toISOString()});
    const t = setInterval(()=> setRemaining(s=> Math.max(0,s-1)), 1000); return ()=> clearInterval(t);
  },[running]);
  useEffect(()=>{
    if(running && remaining===0){ const d = drills.find(x=>x.id===running); onEvent?.("drill.completed",{drillId:d?.id,title:d?.title,ts:new Date().toISOString()}); setRunning(null); }
  },[remaining,running]);
  function add(){ if(!form.title) return; onAdd({ id: Math.random().toString(36).slice(2), ...form, durationSec:Number(form.durationSec)}); setForm({title:"", durationSec:60, goal:""}); setShowNew(false); }
  return (
    <div className="grid grid-3 grid">
      <div className="card" style="grid-column:span 2;">
        <div className="row" style="margin-bottom:8px;justify-content:space-between;"><div style="font-weight:600;">Practice Drills</div><button className="btn" onClick={()=>setShowNew(true)}>+ New Drill</button></div>
        <div className="grid grid-2">
          {drills.map(d=>(
            <div key={d.id} className="card">
              <div style="font-weight:600;">{d.title}</div>
              <div className="muted">Goal: {d.goal || "—"}</div>
              <div className="row" style="margin-top:8px;justify-content:space-between;">
                <div className="muted">Duration: {d.durationSec||60}s</div>
                {running===d.id ? <span className="pill">{remaining}s</span> : <button className="btn" onClick={()=>setRunning(d.id)}>Start</button>}
              </div>
            </div>
          ))}
        </div>
      </div>
      <div className="card"><div style="font-weight:600;margin-bottom:6px;">Coach’s Tip</div><ul style="margin:0 0 0 18px;"><li>Lead with discovery, not features.</li><li>Mirror & label; bridge to value.</li><li>Secure time-bound next steps.</li></ul></div>
      {showNew && (
        <div className="card" style="grid-column:span 3;">
          <div className="row" style="margin-bottom:8px;justify-content:space-between;"><div style="font-weight:600;">New Drill</div><button className="btn" onClick={()=>setShowNew(false)}>Close</button></div>
          <div className="grid grid-2">
            ...
            <label style="grid-column:span 2;">Goal<input value={form.goal} onChange={e=>setForm({...form,goal:e.target.value})}/></label>
          </div>
          <div style="margin-top:8px;"><button className="btn primary" onClick={add}>Save Drill</button></div>
        </div>
      )}
    </div>
  );
...
function Objections({data, onAdd, onDelete}){
  ...
  useEffect(()=>{ if(data.length===0){ setIdx(0); setFlipped(false); return;} if(idx > data.length-1){ setIdx(data.length-1); setFlipped(false);} }, [data.length]);
  const card = data[idx] || {};
  function add(){ if(!form.label || !form.response) return; onAdd(form); setForm({label:"", framework:"", response:""}); setIdx(0); setFlipped(false); }
  return (
    <div className="grid grid-3 grid">
      <div className="card" style="grid-column:span 2;">
        <div className="row" style="margin-bottom:8px;justify-content:space-between;"><div style="font-weight:600;">Objection Flashcards</div><div className="muted">Click the card to flip</div></div>
        {data.length===0 ? (
          ...
          <div className="row" style={{gap:8, alignItems:'stretch'}}>
            ...
            <div className="flip-wrap">
              <div className={"flip-card "+(flipped?"flipped":"")} onClick={()=>setFlipped(v=>!v)}>
                <div className="flip-face flip-front"><div className="muted">Objection</div><h4 style="margin:6px 0;">{card.label}</h4></div>
                <div className="flip-face flip-back"><div className="muted">Framework: {card.framework || "—"}</div><div style="margin-top:8px;font-size:16px;line-height:1.35;">{card.response}</div></div>
              </div>
            </div>
            <button className="btn" onClick={()=>{ setIdx((idx+1)%data.length); setFlipped(false); }}>▶</button>
          </div>
        )}
        {data.length>0 && <div style="text-align:right;margin-top:8px;"><button className="btn" onClick={()=> onDelete(idx)}>Delete current</button></div>}
      </div>
      <div className="card">
        <div style="font-weight:600;margin-bottom:6px;">Add New Objection</div>
        ...
        <button className="btn primary" onClick={add} style="margin-top:8px;">Save Objection</button>
      </div>
    </div>
  );
}
...
function Scorecards({scorecards, onSave}){
  ...
  function submit(){ const rec = {...form, ...crit, total}; onSave(rec); setForm({rep:"", account:"", date:new Date().toISOString(), notes:""}); setCrit({discovery:0, pain:0, fit:0, next:0, confidence:0}); }
  return (
    <div className="grid grid-3 grid">
      <div className="card" style="grid-column:span 2;">
        <div style="font-weight:600;margin-bottom:8px;">Call Review Scorecard</div>
        <div className="grid grid-2">
        ...
        <div className="grid grid-2" style="margin-top:8px;">
      ...
      <div className="card"><div style="font-weight:600;margin-bottom:6px;">Recent Reviews</div>
        <div className="list">
          {scorecards.length===0 ? <div className="muted">Nothing here yet.</div> :
            scorecards.slice(0,10).map((r,i)=>(
              <div key={i} className="card" style="padding:8px;">
                <div style="font-weight:600;">{r.rep||"Rep"} → {r.account||"Account"}</div>
...
function Integrations({connections,setConnections,webhooks,setWebhooks,intLogs,clearLogs}){
  ...
  function toggle(p){
    ...
    if(c?.status==="connected"){ const next = {...connections}; delete next[p.key]; setConnections(next); }
    else { setConnections({...connections, [p.key]: {status:"connected", name:p.name, createdAt:new Date().toISOString(), token: p.auth==="apiKey" ? (apiKeys[p.key]||"<api-key>") : "<oauth-token>"}}); }
  }
  ...
  return (
    <div className="grid grid-3 grid">
      <div className="card" style="grid-column:span 2;">
        <div className="row" style="gap:8px;margin-bottom:8px;"><div style="font-weight:600;">Connections</div></div>
            ...
            return (
              <div key={p.key} className="card">
                <div className="row" style="justify-content:space-between;">
                  <div><div style="font-weight:600;">{p.name}</div><div className="muted">{p.kind} • {p.auth.toUpperCase()}</div></div>
                  <span className="pill">{connected?"Connected":"Not connected"}</span>
                </div>
                {p.auth==="apiKey" && (<label style="margin-top:6px;">API Key<input placeholder="paste key here" value={apiKeys[p.key]||""} onChange={e=>setApiKeys({...apiKeys,[p.key]:e.target.value})}/></label>)}
                <div style="margin-top:8px;"><button className={"btn "+(connected?"":"primary")} onClick={()=>toggle(p)}>{connected?"Disconnect":(p.auth==="oauth2"?"Connect":"Save & Connect")}</button></div>
              </div>
            );
          })}
        </div>
      ...
      <div className="card">
        <div className="row" style="gap:8px;margin-bottom:6px;"><div style="font-weight:600;">Outbound Webhooks</div></div>
        <div className="muted" style="margin-bottom:6px;">Events posted to <code>/api/webhooks</code> by default.</div>
        <div className="list" style="margin-top:8px;">
          <div className="muted">Tip: You can also add external URLs here and forward from your server.</div>
        </div>
      </div>
      <div className="card" style="grid-column:span 3;">
        <div style="font-weight:600;margin-bottom:6px;">Integration Activity</div>
        <div className="muted" style="margin-bottom:6px;">Most recent first</div>
        <div className="list" id="int-log"></div>
        <div style="text-align:right;margin-top:8px;"><button className="btn" onclick="localStorage.removeItem('sc_intLogs');location.reload()">Clear</button></div>
      </div>
    </div>
  );
}
...
function Developers(){
  return (
    <div className="grid grid-2 grid">
      <div className="card"><div style="font-weight:600;margin-bottom:6px;">Events & Payloads</div>
        <pre style="white-space:pre-wrap;background:#f8fafc;padding:8px;border:1px solid var(--border);border-radius:8px;margin-top:8px;">
...
drill.completed -> { "drillId", "title", "ts" }
      ...
      <div className="card"><div style="font-weight:600;margin-bottom:6px;">Webhook Receiver</div>
        <pre style="white-space:pre-wrap;background:#f8fafc;padding:8px;border:1px solid var(--border);border-radius:8px;">
POST /api/webhooks
...
function Settings({onReset}){
  return (
    <div className="card">
      ...
      <div className="row" style="justify-content:space-between;border:1px solid var(--border);padding:8px;border-radius:12px;">
        <div><div style="font-weight:600;">Reset demo data</div><div className="muted">Clears local storage and restores defaults</div></div>
